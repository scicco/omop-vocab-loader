...
def run_ddls(ddl_files, connection_details):
    ...
    try:
        conn = psycopg2.connect(
            ...
            sslrootcert=connection_details["sslrootcert"],
            sslcert=connection_details["sslcert"],
            sslkey=connection_details["sslkey"],
        )
        print("Connection established successfully with mTLS.")
        folder_path = './ddl'
...
def insert_statement_for(csv_filename, table_name, cols):
    ...
    if "concept.csv" in csv_filename.lower():
        ...
        query += "ON CONFLICT (concept_id_1, concept_id_2, relationship_id, valid_start_date,valid_end_date,invalid_reason) DO NOTHING;"
        ...
        query += "ON CONFLICT (concept_id, concept_synonym_name, language_concept_id) DO NOTHING;"
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            for chunk in pd.read_csv(
                ...
                if csv.lower() == "concept_relationship.csv" and 'invalid_reason' in chunk.columns:
                    chunk['invalid_reason'] = chunk['invalid_reason'].replace('', np.nan).fillna('').astype(str)
...
if __name__ == '__main__':
    ...
    connection_details = {
        ...
        "dbname": env['DATABASE'],
        "sslmode": env.get("SSL_MODE"),
        "sslrootcert": env.get("CA_FILE_PATH"),
        "sslcert": env.get("CERT_FILE_PATH"),
        "sslkey": env.get("CERT_FILE_KEY")
    }
    ...
    if env.get('CREATE_TABLES', False) in ['True', 'true', True, '1', 1]:
        ...
        run_ddls(ddl_files, connection_details)
        run_ddls(['7_add_vocab_uniques.sql'], connection_details)
